...
#---------------------------   EXPORT FUNCTIONS   -----------------------------
#Functions to export data, either to text file (writeData), KML format 
#(exportKmlN) or ESRI shapefile (exportShp). The text file format can also be
#read back into PyTrx using the import functions.
           
    def writeData(self, dest):
        '''Write all data (if it has been calculated) to separate files.
        
        Inputs
        -Destination: folder directory where output files will be written to 
        (NOT a specific file)
        
        Outputs
        -Cumulative pixel extent: written as a single text file containing 
         delineated values of total pixel area recorded for all images in the 
         sequence
        -Polygon pixel coordinates: written as a tab delimited text file 
         containing polygon xy coordinates
        -Polygon areas: Real world area for each polygon in an image sequence
        -Cumulative area: written as a single text file containing the total
         area of all the polgons from each image
    ...
    def exportKmlN(self, number, dest):
        #Get real polygons if they have not been previously calculated
        if self._realpoly is None:
            self.calcAreas
        
        #Get polygons for specific image number
        xyz = self._realpoly[number]

        #Transform to OGR compatible polygons and export to KML format
        poly = self.ogrPolyN(xyz)
        
        #Transform each polygon to KML format
        count=1
        for p in poly:
            kml = p.ExportToKML()
            target = dest + 'poly' +str(count) + '.kml'
            f = open(target, 'w')
            f.write(kml)
            count=count+1

         
    def exportSHP(self, fileDirectory, projection=None):
        ...
        -projection: Coordinate projection that the shapefile will exist in.
        This can either be an ESPG number (expressed as an integer) or a well-
        known geographical coordinate system (expressed as a string).
        Well-known geographical coordinate systems are:
        'WGS84', 'WGS72', NAD83' or 'EPSG:n'
        Search for ckassOGRSpatialReference at http://www.gdal.org/ for more 
        information''' 
...
#----------------------------  IMPORT FUNCTIONS   -----------------------------
#Functions for importing pixel area data and real world data from text file.
...
#and area text files must be named correctly - 'area_coords.txt', 
#'area_all.txt', 'px_coords.txt', and 'px_sum.txt'.
    ...
    def importData(self, fileDirectory):
        ...
        rpoly, rarea = self.importXYZ(fileDirectory)
        pxpoly, pxarea = self.importPX(fileDirectory)
        self.setMethod('auto')
...
#read back into PyTrx using the import functions.
    ...
    def writeData(self, dest):
        ...
        Outputs
        -Pixel coordinates of lines: written as a single text file containing 
         line xy coordinates for all images in the sequence
        -Pixel line lengths: contains pixel length of each line in all images
        -Real coordinates: written as a tab delimited text file 
         containing line xyz coordinates
        -Real line lengths: Real world length of each line in all images
         
        All these file types are compatible with the importing tools 
        (importPX, importXYZ)''' 
        #Pixel line coordinates file generation             
        if self._pxpts is not None:            
    ...
    def exportSHP(self, fileDirectory, projection=None):
        ...
        Well-known geographical coordinate systems are:
        'WGS84', 'WGS72', NAD83' or 'EPSG:n'
        See http://www.gdal.org/classOGRSpatialReference.html#a096b8dde4fd2eb475acd376060940b02
